...
# ======== Real + Sim file watcher + timeseries API ========
...
class _ResultsEventHandler(FileSystemEventHandler):
    ...
    def _notify(self, event):
        ...
        # only notify on final artifacts; ignore temp files
...
def sse_files():
    """SSE: emits a 'file' event when a result file is created/modified."""
    @stream_with_context
    def gen():
        yield "event: ping\ndata: {}\n\n"
        while True:
