...
class OpenDCRunner:
    ...
    def create_enhanced_mock_results(self, tasks_data=None, fragments_data=None, outdir="/app/data"):
        """
        Create a realistic mock OpenDC run:
          - Writes powerSource.parquet, host.parquet, service.parquet under outdir
          - Returns summary: {energy_kwh, cpu_utilization, max_power_draw, runtime_hours, status}
        """
        ...
        # --- generate 24h @ 5min cadence ---
        step_min = 5
        points = 24 * 60 // step_min
        start = datetime.now(timezone.utc) - timedelta(minutes=step_min * (points - 1))
        ts = [start + timedelta(minutes=step_min * i) for i in range(points)]

        # CPU utilization (0..1) with daily-ish pattern + noise
        t = np.linspace(0, 2 * math.pi, points)
        cpu = np.clip(0.55 + 0.25 * np.sin(4 * t) + 0.08 * np.random.randn(points), 0, 1)

        # Energy per 5-min bucket (kWh), scaled by CPU + noise; then convert to Joules
        kwh_bucket = np.clip(cpu * 0.70 / 12 + 0.008 * np.random.randn(points), 0, None)
        energy_J = (kwh_bucket * 3_600_000).astype(float)

        # Average power ~ kWh_bucket * 12 → kW → convert to W
        power_kw = kwh_bucket * 12.0
        power_w = (power_kw * 1000.0).astype(float)

        # --- build frames ---
        df_power = pd.DataFrame({
            "timestamp": ts,
            "energy_usage": energy_J,       # Joules
            "power_draw": power_w           # Watts
        })
        df_host = pd.DataFrame({
            "timestamp": ts,
            "cpu_utilization": cpu          # 0..1
        })
        df_service = pd.DataFrame({"timestamp": ts})

        # --- write parquet artifacts ---
        power_path = os.path.join(outdir, "powerSource.parquet")
        host_path = os.path.join(outdir, "host.parquet")
        service_path = os.path.join(outdir, "service.parquet")
        df_power.to_parquet(power_path, index=False)
        df_host.to_parquet(host_path, index=False)
        df_service.to_parquet(service_path, index=False)

        # --- summary output expected by your app ---
        energy_kwh_total = float(kwh_bucket.sum())
        cpu_mean = float(df_host["cpu_utilization"].mean())
        max_power_draw = float(df_power["power_draw"].max())
        runtime_hours = points * step_min / 60.0
