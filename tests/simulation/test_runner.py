...
def test_run_simulation_rejects_non_executable(tmp_path, monkeypatch):
    """A discovered binary must be executable before launching the simulation."""

    runner = OpenDCRunner()

    non_exec_path = tmp_path / "OpenDCExperimentRunner"
    non_exec_path.write_text("echo noop")
    # Ensure the file lacks execute permissions
