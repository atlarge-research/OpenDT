...
function renderMetrics(s) {
  ...
  $("#mFragments") &&
    ($("#mFragments").textContent = frags ?? s.total_fragments ?? 0);

  const sim = s.last_simulation || {};
  $("#mPower") && ($("#mPower").textContent = fmt(sim.mean_power_draw_kw, 2));
  ...
  const action =
    opt.action_taken ||
    (Array.isArray(opt.action_type) ? opt.action_type[0] : null) ||
    "—";
...
function renderOpenDC(sim, s) {
  ...
  $("#kvSimType") &&
    ($("#kvSimType").textContent = fmt(s.cycle_count_opt) ?? "—");
      ...
      (c.hosts || []).forEach((h) => {
        rows.push(`
          <tr>
            ...
            <td>${
              Math.round((h.memory?.memorySize || 0) / 1073741824) || "—"
            }</td>
          </tr>
        `);
      });
...
function renderLLM(opt) {
  ...
  if (
    ...
    typeof window.recommendationEditor.ingest === "function"
  ) {
